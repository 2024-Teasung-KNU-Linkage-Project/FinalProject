...
class AnalysisModal extends Modal {
  ...
  modal_init(fileName, chemicalIntegratedData, odorPredictData) {
    for (let chemical of chemicalIntegratedData){
      console.log(chemical.chemicalName)
    }
    ...
    if (fileName != null) {
        // solution-button div 선택
        const solutionButtonDiv = document.getElementById("solution-button");
        if (solutionButtonDiv) {
            // 클릭 이벤트 리스너 추가
            solutionButtonDiv.onclick = () => {
                this.redirectWithData(fileName);
            };
        }
    }
    /*
    if (fileName != null){
      ...
      redirectButton.textContent = "데이터 전송";
      redirectButton.style.marginTop = "20px";
      redirectButton.onclick = () => this.redirectWithData(fileName);

      // 모달에 버튼 추가
      const modalFooter = document.getElementById("odorPredictTable"); // 모달 하단에 추가할 곳 선택
      if (modalFooter) {
        const existingButtons = modalFooter.querySelectorAll("button");
        existingButtons.forEach(button => button.remove());
        modalFooter.appendChild(redirectButton);
      }
    } */
  }
  // chemicalIntegratedData와 함께 URI로 리다이렉트
  redirectWithData(fileName) {
    const baseURL = "http://localhost:8081/soms/page5/input"; // 리다이렉트할 URI
    const targetURL = `${baseURL}?fileName=${fileName}`;
    // 페이지 리다이렉트
  ...
  generateIntegratedChemicalRows(data) {
    ...
    for (let i = 0; i < data.length || i < 10; i++) {
      ...
      const chemicalValue = data[i]?.chemicalValue.toFixed(1) || "-";
      const minimumValue = data[i]?.minimumValue.toFixed(1) || "-";
      const dilutionRate = data[i]?.dilutionRate.toFixed(1) || "-";
      const relativeRatio = data[i]?.relativeRatio.toFixed(1) || "-";
